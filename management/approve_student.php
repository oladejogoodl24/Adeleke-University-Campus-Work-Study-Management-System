...
// Admin-only access
if (!isset($_SESSION['management_role']) || $_SESSION['management_role'] != 'admin') {
    header("Location: dashboard.php?error=Admin access required");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'approve') {
        ...
        // Note: Add rejection_reason column to students table if needed
        $stmt = $conn->prepare("UPDATE students SET status = ? WHERE id = ?");
        $stmt->bind_param("si", $status, $student_id);
    } else {
    ...
    if ($stmt->execute()) {
        header("Location: approve_student.php?success=" . urlencode($message));
    } else {
        header("Location: approve_student.php?error=Database error");
    }
    exit();
}
...
<title>Approve Students - Management Dashboard</title>
<style>
body {
    margin: 0;
...
<div class="main">
        ...
        if ($students->num_rows > 0) {
            while ($row = $students->fetch_assoc()) {
                echo "<div style='border-bottom: 1px solid #334155; padding: 15px 0;'>
                        ...
                        <span style='color: #94a3b8;'>Matric: {$row['matric_number']}</span><br>
                        <span style='color: #94a3b8;'>Department: {$row['department']} | Level: {$row['level']}</span><br>
                        <span style='color: #94a3b8;'>Scholarship: {$row['scholarship_status']}</span><br><br>
                        <form method='POST' action='approve_student.php' style='display:inline;'>
                            <input type='hidden' name='student_id' value='{$row['id']}'>
                            <input type='hidden' name='action' value='approve'>
                            <button type='submit'>Approve</button>
                        </form>
                        <form method='POST' action='approve_student.php' style='display:block; margin-top: 10px;'>
                            <input type='hidden' name='student_id' value='{$row['id']}'>
    ...
    <div class="card">
        ...
        <?php
        $approved_students = $conn->query("SELECT * FROM students WHERE status='approved' ORDER BY full_name");

        if ($approved_students->num_rows > 0) {
            while ($row = $approved_students->fetch_assoc()) {
                echo "<div style='border-bottom: 1px solid #334155; padding: 10px 0;'>
                        <strong>{$row['full_name']}</strong> - {$row['email']} ({$row['matric_number']})
                      </div>";
            }
        } else {
            echo "<p>No approved students yet.</p>";
        }
        ?>
    </div>
